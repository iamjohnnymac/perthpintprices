    ...
    <div className="min-h-screen bg-cream">
      ...
      <div className="bg-white border-b border-stone-200 sticky top-0 z-50">
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2 text-ocean hover:text-cyan-700 transition-colors font-semibold text-sm">
            <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
              <path d="M19 12H5M12 19l-7-7 7-7"/>
            </svg>
            Back to PintDex
          </Link>
          <button onClick={sharePub} className="text-stone-400 hover:text-ocean transition-colors p-2" title="Share this pub">
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-5">
        ...
        <div className="space-y-3">
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1 min-w-0">
              ...
              {distance && <p className="text-ocean text-sm font-medium mt-0.5">{distance}</p>}
            </div>
          ...
          <div className="flex flex-wrap gap-2">
            {pub.isHappyHourNow && (
              <Badge className="bg-teal hover:bg-teal text-white animate-pulse">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <Card className="border-stone-200/60 shadow-sm rounded-xl">
            <CardContent className="p-4">
              ...
              <a
                ...
                className="inline-flex items-center gap-1 mt-2 text-ocean text-xs font-semibold hover:text-cyan-700 transition-colors"
              >
          ...
          {(pub.happyHour || pub.happyHourPrice) && (
            <Card className={`border-stone-200/60 shadow-sm rounded-xl ${pub.isHappyHourNow ? 'ring-2 ring-teal/30 bg-teal/5' : ''}`}>
              <CardContent className="p-4">
                <div className="text-xs text-stone-400 uppercase tracking-wider font-semibold mb-1">Happy Hour</div>
                {pub.happyHourPrice && (
                  <p className={`text-lg font-bold font-mono ${pub.isHappyHourNow ? 'text-teal' : 'text-stone-700'}`}>
        ...
        <div className="flex gap-3">
          {pub.website && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex-1 bg-ocean text-white text-center py-3 rounded-xl font-semibold text-sm hover:bg-cyan-700 transition-colors"
            >
              Visit Website →
            </a>
          )}
                    ...
                    <CardContent className="p-4 flex items-center justify-between">
                      <div className="min-w-0 flex-1">
                        ...
                        {nearby.isHappyHourNow && (
                          <p className="text-xs text-teal font-semibold mt-0.5">{E.party} Happy Hour Live</p>
                        )}
                      </div>
        ...
        <div className="text-center py-6 text-xs text-stone-400">
          ...
          <p className="mt-1">
            Wrong price?{' '}
            <Link href="/" className="text-ocean hover:text-cyan-700 font-semibold">
