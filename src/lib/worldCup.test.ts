...
describe('WC_FIXTURES data sanity', () => {
  ...
  it('keeps semi-final placeholders only where FIFA has not confirmed both teams', () => {
    const semiFinals = WC_FIXTURES.filter(f => f.round === 'Semi-final')

    assert.deepEqual(
      semiFinals.map(f => [f.id, f.kickoff, f.home, f.away]),
      [
        ['2026-07-15-sf-m101', '2026-07-15T03:00:00+08:00', 'France', 'Spain'],
        ['2026-07-16-sf-m102', '2026-07-16T03:00:00+08:00', 'England', 'Winner M100'],
      ],
    )
  })
