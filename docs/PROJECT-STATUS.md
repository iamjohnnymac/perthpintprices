...
PR #201 (+ image follow-up #202). Worked the mobile PageSpeed report. The PSI API was quota-blocked at the project level (`defaultPerDayPerProject` = 0), so audits were captured by running `lighthouse` directly against the live URL. **Live production after deploy: Performance 72→88, Accessibility 77→100, Best Practices 96→100, SEO 100 (unchanged); FCP 3.6s→1.8s, LCP 5.3s→3.5s, CLS 0.001.**
