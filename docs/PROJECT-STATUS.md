...
- **Retired-URL hygiene:** legacy `/pub/{slug}` misses (removed/re-slugged venues: badlands-bar, jack-rabbit-slims, w-churchill, wolf-lane, ruin-bar, the-flying-scotsman) now return **`410 Gone` instead of `404`** so Google drops the dead `/pub/*` namespace faster; the `301` redirect still fires when the slug resolves. The current-structure 404s (`/northbridge/ruin-bar`, `/mount-lawley/five-bar`, `/perth-cbd/halford-bar`) are genuinely-removed pubs — `404` is correct, they'll drop naturally. The 4 `Duplicate canonical` (incl. `/suburb/north-perth`, which 301s cleanly) and the `/guides`+`/insights` redirect-error validations are stale classifications that clear on recrawl.
