...
- **Issue #250 / commit `5159619` / branch `codex/andrew-admin-demo`:** adds a finished, unlisted and `noindex` Andrew price-check presentation at `/ai-price-demo`, plus an authenticated Andrew admin tab with a masked fixed destination, explicit AI-call/transcription consent, live status, sanitized transcript and a proposed listing preview.
...
- **Issue #237 / commit `e793122` / Phase 1 verification:** production sitemap crawl passed for all 1,015 emitted URLs (32 content, 150 suburbs and 833 legitimate pubs): every URL was `200`, self-canonical and index-eligible, with no duplicates or prohibited routes. The corrected sitemap index was re-submitted successfully in authenticated GSC; the 10 July Page-indexing report has not recrawled it yet, so this begins the observation window rather than claiming recovery. Sanitized evidence and the weekly closure gate are in `docs/seo/index-quality-verification-2026-07-21.md`.
...
- **Technical reconciliation:** the one historic Henley Brook canonical mismatch, two intentional section-root redirects, and every one of the historical 15 reported 404 URLs are inventoried alongside #236's production handling. Terminal proof and a faithful inspected PNG are in `artifacts/gsc-indexing-229/`.
- **Refresh guardrails:** the baseline now requires explicit report/export dates and an output directory, derives live inventory and segmented-sitemap totals, and records no stored URL-Inspection example unless supplied as dated source evidence. It rejects malformed, missing, duplicate, dropped, and unjoined current pub-route rows; regression tests cover each failure class. Initial-HTML usefulness is now evidence-based (H1, visible path links, and ItemList data or methodology) for every row, including all seven day pages and four transport hubs.
...
- **Verification:** 371 unit tests, TypeScript, lint, redirect checks, the production build, and 10 desktop/mobile Playwright PR-proof checks passed. The browser regression covers the sitemap index and all three child XML feeds without timestamp coupling. A built local crawl verified every one of 1,015 emitted URLs: 200 response, self-canonical, zero duplicates, zero prohibited URLs, and exactly 833 legitimate pubs. Raw/PNG terminal evidence: `artifacts/segmented-sitemap-232/terminal-proof.*`.
...
- **Review evidence follow-up / commit `d19c73b`:** replaced the declarative matrix with a captured PR-preview terminal proof covering every reported route, canonical, title, legacy-suburb, and `www` apex response. Added a local production-route Playwright regression for the Henley Brook canonical/title and intentional `410`/`404` responses.
...
- Suburb pages now render every legitimate pub link in the initial HTML inside a visible scrollable list, retain nearby-suburb links and add high-intent comparison links. The old `sr-only` / `aria-hidden` crawler blocks were removed after the visible paths replaced them.
- **Reviewer follow-up / commit `56f57ac`:** canonical eligibility is now applied before Discover's slim payload removes business status; Tier-C checked-price fallbacks exclude confirmed closures while legitimate missing and unverified-price pubs remain linkable; the PR proof covers the new headings and destinations; and all new contextual/ranking links meet the 48px mobile touch-target floor.
- **Verification:** 358 unit tests, 9 focused link-policy tests, TypeScript, lint, redirect and security-header contracts, the Infisical-backed production build against 849 pubs, and 8 desktop/mobile Chromium PR-proof checks pass. Raw HTML contained all five data-tool links, TBC pub recommendations and all 63 legitimate Fremantle pub links before hydration. Desktop/mobile before-and-after evidence is in `artifacts/visible-link-rails-233/`.

### Every legitimate pub remains indexable (2026-07-21)
- **Issue #230 / commit `7b32b55`:** superseded the earlier Tier-C noindex policy. Price availability, verification, and freshness now classify the page's trust/presentation state only; they do not hide a pub, change its canonical, emit `noindex`, or exclude it from the sitemap.
- **Inventory reconciliation:** the Infisical-backed read-only baseline found 849 routable pub rows: 833 legitimate pubs and 16 Google-confirmed permanent closures. Before this change only 233 legitimate pubs were sitemap/index eligible; the preservation-first contract keeps all 833 eligible, including 634 missing-price pubs and 13 unverified-price pubs. Price recency across legitimate pubs was 0 fresh, 10 aging, 191 stale, and 632 unknown on 2026-07-21.
- The honest missing-price state, visible report-price action, contextual venue links, self-canonical metadata, and schema guards remain in place. `CLOSED_PERMANENTLY` remains a separate evidence-based noindex/sitemap exception; duplicate and invalid venue cleanup stays in the reviewed data workflow rather than using price quality as a proxy.
- **Reviewer follow-up / commit `3b51f77`:** extracted the production pub metadata builder and sitemap route assembly into directly tested boundaries. The regression matrix now proves fresh, aging, stale, missing-date, missing-price, and unverified pubs emit `index,follow`, keep their self-canonical, and reach the pub sitemap; a confirmed permanent closure remains noindexed and sitemap-excluded.
- **Verification:** all 352 unit tests, TypeScript, lint, and the Infisical-backed production build pass. The generated pub-sitemap feed reconciles exactly to 833 legitimate pubs (173 Tier A, 60 Tier B, 600 Tier C), with zero legitimate omissions and zero confirmed closures included. A built HTTP check of the real Tier-C `/fremantle/federal-hotel` route returned 200, `robots=index, follow`, its self-canonical, honest `Price TBC` / `No verified price` content, a visible `Report a price` action, and sitemap membership. No visible UI changed, so screenshot evidence is not applicable.
