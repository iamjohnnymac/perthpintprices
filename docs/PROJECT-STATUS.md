...
- **Issue #250 / implementation commit `0ffd87f` / branch `codex/andrew-admin-demo`:** adds an unlisted, `noindex` Andrew price-check presentation at `/ai-price-demo`, plus an authenticated Andrew admin tab with a masked fixed destination, explicit AI-call/transcription consent, live status, a sanitized transcript and a proposed listing preview. The public worked example now names an obviously fictitious venue and makes no publication or independent-verification claim.
- **No-write and privacy boundary:** both production and demo agent IDs are required and must differ. The endpoint accepts consent only, reads the destination from `AI_DEMO_TEST_PHONE_E164`, holds a unique database sentinel for the call's lifetime, reclaims only an expired matching sentinel, and enforces a 15-minute cooldown. Both polling and post-call webhooks sanitize vendor-controlled transcript, collection, tool-result and failure data; demo callbacks cannot query `pubs` or write `pubs` / `price_history`.
- **Reviewable demo assets:** `agents/andrew-demo.json` records the proposed separate-agent contract without applying it to ElevenLabs. `public/audio/andrew-price-check.mp3` was generated by the checked-in script from the existing Andrew voice settings and is exercised through play, replay, reset and reduced-motion flows.
- **Owner gates remain:** no test call was placed and no live agent configuration was changed. Before a live owner test, create and review the separate demo agent, confirm recording is disabled and the one-day provider-retention policy is acceptable, configure or rotate the three required secrets, and verify the reserved-slug tool contract in ElevenLabs.
- **Verification:** all 391 unit tests, TypeScript, lint, the production build and four production-mode Playwright checks pass. Inspected evidence under `artifacts/issue-250/` compares the authenticated `origin/main` admin view and missing public route with this branch using identical Playwright viewports and device pixel ratios. The humanizer skill was unavailable, so new copy was checked manually against `docs/brand-voice-brief.md`.
