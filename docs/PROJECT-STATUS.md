...
- **Review evidence follow-up / commit `COMMIT_PENDING`:** replaced the declarative matrix with a captured PR-preview terminal proof covering every reported route, canonical, title, legacy-suburb, and `www` apex response. Added a local production-route Playwright regression for the Henley Brook canonical/title and intentional `410`/`404` responses.
